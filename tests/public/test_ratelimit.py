...
def test_register_get_never_rate_limited() -> None:
    """GET requests to register are never rate-limited (limit is POST-only)."""
    url = reverse("public:register")
...
# ---------------------------------------------------------------------------
# register_details_form partial — require_htmx still active (Invariant 7)
# ---------------------------------------------------------------------------


def test_register_details_form_partial_rejects_non_htmx() -> None:
    """The HTMX-only register_details_form partial returns 400 for plain requests."""
    url = reverse("public:register_details_form")
    client = Client()
    response = client.get(url + "?role=ambassador")
